...
import (
	...
	"github.com/Kevionte/go-sovereign/internal/blocktest"
...
func newTestBackend(t *testing.T, n int, gspec *core.Genesis, engine consensus.Engine, generator func(i int, b *core.BlockGen)) *testBackend {
	...
	gspec.Alloc[acc.Address] = types.Account{Balance: big.NewInt(params.Swc)}
...
func TestEstimateGas(t *testing.T) {
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				accounts[0].addr: {Balance: big.NewInt(params.Swc)},
				accounts[1].addr: {Balance: big.NewInt(params.Swc)},
			},
		}
		genBlocks      = 10
	...
	var testSuite = []struct {
		...
		},
		{
			...
			call:        TransactionArgs{},
			overrides: StateOverride{
				randomAccounts[0].addr: OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Swc)))},
			},
			expectErr: nil,
			want:      53000,
		},
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				accounts[0].addr: {Balance: big.NewInt(params.Swc)},
				accounts[1].addr: {Balance: big.NewInt(params.Swc)},
				accounts[2].addr: {Balance: big.NewInt(params.Swc)},
				dad: {
					Balance: big.NewInt(params.Swc),
					Nonce:   1,
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	...
	var (
		...
		genesis    = &core.Genesis{
			Config: params.TestChainConfig,
			Alloc: types.GenesisAlloc{
				acc1Addr: {Balance: big.NewInt(params.Swc)},
				acc2Addr: {Balance: big.NewInt(params.Swc)},
			},
		}
		genBlocks = 10
		signer    = types.HomesteadSigner{}
	...
	var (
		...
		genesis    = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				//     }
				// }
				contract: {Balance: big.NewInt(params.Swc), Code: common.FromHex("")},
			},
		}
