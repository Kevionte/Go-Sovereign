...
func TestSupplyGenesisAlloc(t *testing.T) {
	var (
		...
		eth1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Swc))
	...
	expected := supplyInfo{
		Issuance: &supplyInfoIssuance{
			GenesisAlloc: (*hexutil.Big)(new(big.Int).Mul(common.Big2, big.NewInt(params.Swc))),
		},
		Number:     0,
...
func TestSupplyRewards(t *testing.T) {
	...
	expected := supplyInfo{
		Issuance: &supplyInfoIssuance{
			Reward: (*hexutil.Big)(new(big.Int).Mul(common.Big2, big.NewInt(params.Swc))),
		},
		Number:     1,
...
func TestSupplyEip1559Burn(t *testing.T) {
	...
	var (
		...
		reward   = new(big.Int).Mul(common.Big2, big.NewInt(params.Swc))
...
func TestSupplySelfdestructItselfAndRevert(t *testing.T) {
	var (
		...
		gwei5   = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.GWei))
		eth1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Swc))
		eth2    = new(big.Int).Mul(common.Big2, big.NewInt(params.Swc))
		eth5    = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.Swc))
