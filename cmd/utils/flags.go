...
import (
	...
	bparams "github.com/Kevionte/Go-Sovereign/beacon/params"
	...
	"github.com/Kevionte/Go-Sovereign/ethdb/remotedb"
	"github.com/Kevionte/Go-Sovereign/ethstats"
	"github.com/Kevionte/Go-Sovereign/graphql"
	...
	"github.com/Kevionte/Go-Sovereign/metrics/exp"
	"github.com/Kevionte/Go-Sovereign/metrics/influxdb"
	...
	"github.com/Kevionte/Go-Sovereign/p2p/nat"
...
var (
	...
	HoleskyFlag = &cli.BoolFlag{
		Name:     "testnet",
		Usage:    "Sovereign test network: pre-configured proof-of-stake test network",
		Category: flags.EthCategory,
	}
	// Dev mode
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "SWC", ethPeers, "total", cfg.MaxPeers)
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, GoerliFlag, HoleskyFlag)
...
// RegisterEthService adds an Sovereign client to the stack.
// The second return value is the full node instance.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Ethereum) {
	backend, err := eth.New(stack, cfg)
	if err != nil {
		Fatalf("Failed to register the Sovereign service: %v", err)
	}
	stack.RegisterAPIs(tracers.APIs(backend.APIBackend))
	return backend.APIBackend, backend
}

// RegisterEthStatsService configures the Sovereign Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Sovereign Stats service: %v", err)
	}
}
